...
import { GitlabPRDSL, GitlabJSONDSL } from "../dsl/GitlabDSL"
import { GitlabAPI } from "./gitlab/GitlabAPI"
import gitDSLForGitlab from "./gitlab/GitlabGit"
import { Platform, Comment } from "./platform"

import { debug } from "../debug"

/** Handles conforming to the Platform Interface for Gitlab, API work is handle by GitlabAPI */
...
export class Gitlab implements Platform {
   ...
   */
  getReviewInfo = (): Promise<GitlabPRDSL> => this.api.getPullRequestInfo()
   ...
   */
  getPlatformGitRepresentation = (): Promise<GitJSONDSL> => gitDSLForGitlab(this.api)
  ...
  /**
   * Returns the `GitlabJSONDSL` object on the Danger DSL
   *
   * @returns {Promise<GitlabJSONDSL>} JSON response of the DSL
   */
  ...
  /**
   * Finds type of line in given diff. This is needed for Gitlab API
   *
