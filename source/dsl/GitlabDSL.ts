/** Key details about a repo */
export interface RepoMetaData {
  /** A path like "artsy/eigen" */
  repoSlug: string
  /** The ID for the pull/merge request "11" */
  pullRequestID: string
}

// This is `danger.gitlab` inside the JSON

export interface GitlabJSONDSL {
  /** The pull request and repository metadata */
  metadata: RepoMetaData
  /** The related JIRA issues */
  issues: JIRAIssue[]
  /** The PR metadata */
  pr: GitlabPRDSL
  /** The commits associated with the pull request */
  commits: GitlabCommit[]
  /** The comments on the pull request */
  comments: GitlabPRActivity[]
  /** The activities such as OPENING, CLOSING, MERGING or UPDATING a pull request */
  activities: GitlabPRActivity[]
}

// This is `danger.gitlab`

/** The Gitlab metadata for your PR */
export interface GitlabDSL extends GitlabJSONDSL {}

/**
 * This is `danger.gitlab.issues` It refers to the issues that are linked to the Pull Request.
 */
export interface JIRAIssue {
  /** The unique key for the issue e.g. JRA-11 */
  key: string
  /** The user-facing URL for that issue */
  url: string
}

// This is `danger.gitlab.pr`

/**
 * An exact copy of the PR's reference JSON. This interface has type'd the majority
 * of it for tooling's sake, but any extra metadata which Gitlab Server send
 * will still be inside the JS object.
 */

export interface GitlabPRDSL {
  /** The PR's ID */
  id: number
  /** The API version */
  version: number
  /** Title of the pull request. */
  title: string
  /** The text describing the PR */
  description: string
  /** The pull request's current status. */
  state: "OPEN" | "MERGED" | "DECLINED" | "SUPERSEDED"
  /** Is the PR open? */
  open: boolean
  /** Is the PR closed? */
  closed: boolean
  /** Date PR created as number of milliseconds since the unix epoch */
  createdDate: number
  /** Date PR updated as number of milliseconds since the unix epoch */
  updatedDate: number
  /** The PR submitter's reference */
  fromRef: GitlabMergeRef
  /** The repo Danger is running on */
  toRef: GitlabMergeRef
  /** Was this PR locked? */
  locked: boolean
  /** The creator of the PR */
  author: GitlabPRParticipant & { role: "AUTHOR" }
  /** People requested as reviewers */
  reviewers: (GitlabPRParticipant & { role: "REVIEWER" })[]
  /** People who have participated in the PR */
  participants: (GitlabPRParticipant & { role: "PARTICIPANT" })[]
  /** Misc links for hypermedia conformance */
  links: GitlabLinks<"self">
}

// These are the individual subtypes of objects inside the larger DSL objects above.

/** A Gitlab specific implementation of a git commit. */
export interface GitlabCommit {
  /** The SHA for the commit */
  id: string
  /** The shortened SHA for the commit */
  displayId: string
  /** The author of the commit, assumed to be the person who wrote the code. */
  author: {
    /** The id of the commit author */
    name: string
    /** The display name of the commit author */
    displayName: string
    /** The email of the commit author */
    emailAddress: string
  }
  /** The UNIX timestamp for when the commit was authored */
  authorTimestamp: number
  /** The author of the commit, assumed to be the person who commited/merged the code into a project. */
  committer: {
    /** The id of the commit committer */
    name: string
    /** The display name of the commit committer */
    displayName: string
    /** The email of the commit committer */
    emailAddress: string
  }
  /** When the commit was commited to the project */
  committerTimestamp: number
  /** The commit's message */
  message: string
  /** The commit's parents */
  parents: {
    /** The full SHA */
    id: string
    /** The simplified sha */
    displayId: string
  }[]
}

export interface GitlabDiff {
  /** The file refrence when moved */
  destination?: GitlabFile
  /** The original file refrence */
  source?: GitlabFile
  /** A set of diff changes */
  hunks: GitlabHunk[]
  /** If the hunk is massive, then it will be truncated */
  truncated: boolean
  /** The commit SHA which changed this hunk */
  toHash: string
  /** Last SHA where this hunk was changed */
  fromHash: string
  /** The settings for the whitespace */
  whitespace: "SHOW" | "IGNORE_ALL"
}

export interface GitlabFile {
  components: string[]
  name: string
  parent: string
  toString: string
}

export interface GitlabHunk {
  destinationLine: number
  destinationSpan: number
  segments: GitlabSegment[]
  sourceLine: number
  sourceSpan: number
  truncated: boolean
}

export interface GitlabSegment {
  lines: GitlabLine[]
  truncated: boolean
  type: "ADDED" | "REMOVED" | "CONTEXT"
}

export interface GitlabLine {
  source: number
  destination: number
  line: string
  truncated: boolean
  conflictMarker?: "OURS"
  commentIds?: number[]
}

export interface GitlabPRParticipant {
  /*The user for  */
  user: GitlabUser
  /** How did they contribute */
  role: "AUTHOR" | "REVIEWER" | "PARTICIPANT"
  /** Did they approve of the PR? */
  approved: boolean
  /** Their review feedback */
  status: "APPROVED" | "UNAPPROVED" | "NEEDS_WORK"
}

/**
 * A Gitlab user account.
 */
export interface GitlabUser {
  /** The name of the user */
  name: string
  /** The email for the user */
  emailAddress: string
  /** The unique user ID */
  id: number
  /** The name to use when referencing the user */
  displayName: string
  /** Is the account active */
  active: boolean
  /** The user's slug for URLs */
  slug: string
  /** The type of a user, "NORMAL" being a typical user3 */
  type: "NORMAL" | "SERVICE"
}

/**
 * A Gitlab Server Repo
 */
export interface GitlabRepo {
  /** The slug for the repo */
  slug: string
  /** The repo name */
  name?: string
  /** The type of SCM tool, probably "git" */
  scmId: string
  /** Is the repo public? */
  public: boolean
  /** Can someone fork this repo? */
  forkable: boolean
  /** Links for the projects */
  links: GitlabLinks<"self" | "clone">
  /** An abstraction for grouping repos */
  project: {
    /** The project unique id */
    id: number
    /** The project's human readable project key */
    key: string
    /** Is the project publicly available */
    public: boolean
    /** The name of the project */
    name: string
    /** The project's type */
    type: string
    /** Hyperlinks for the project */
    links: GitlabLinks<"self">
  }
}

export type GitlabLinks<Names> = {
  [key in keyof Names]: {
    href: string
    name?: string
  }[]
}

export interface GitlabMergeRef {
  id: string
  displayId: string
  latestCommit: string
  repository: GitlabRepo
}

export interface GitlabPRActivity {
  action: "COMMENTED" | "OPENED" | "MERGED" | "DECLINED" | "UPDATED"
  comment?: GitlabPRComment
  commentAction?: "ADDED" | "UPDATED"
  commentAnchor?: {
    diffType: "COMMIT" | "EFFECTIVE" | "REQUIRED" | "RANGE"
    line: number
    lineType: "CONTEXT" | "ADDED" | "REMOVED"
    fileType: "FROM" | "TO"
    fromHash: string
    path: string
    srcPath: string
    toHash: string
  }
  createdDate: number
  id: number
  user: GitlabUser
}

export interface GitlabPRComment {
  author: GitlabUser
  comments: GitlabPRActivity[]
  createdDate: number
  updatedDate: number
  id: number
  permittedOperations: {
    deletable: boolean
    editable: boolean
  }
  text: string
  version: number
  parent?: {
    id: number
  }
}

export interface GitlabChanges {
  nextPageStart: number | null
  values: GitlabChangesValue[]
}

export interface GitlabChangesValueAddModifyDelete {
  type: "ADD" | "MODIFY" | "DELETE"
  path: {
    toString: string
  }
}

export interface GitlabChangesValueMove {
  type: "MOVE"
  path: {
    toString: string
  }
  srcPath: {
    toString: string
  }
}

export type GitlabChangesValue = GitlabChangesValueAddModifyDelete | GitlabChangesValueMove
